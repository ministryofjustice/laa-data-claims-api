 ...
 * configurations.
 *
 * <p>The tests are started with the single method annoted with {@code @TestTemplate}
 * annotation.</p>
 *
 * <p>Pulls all pacts which are published on the cloud platform
 * <a href="https://laa-data-pact-broker.apps.live.cloud-platform.service.justice.gov.uk/">
 * Pact Broker</a> service which are tagged with the provider name 'laa-data-claims-api'. These
 * pacts have been generated by various dependent services as part of their own testing and
 ...
 * compatible with the latest requirements and changes.</p>
 *
 * <p>By default, all consumer pacts published under their own respected 'main' branch is
 * pulled from the Pact Broker, however these can be overriden using two environment variables:
 * <ul>
 *   <li>{@code PACT_CONSUMER_NAME} - The name of the consumer (can be found on pact broker).</li>
