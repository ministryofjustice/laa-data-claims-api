-- Add the new READY_FOR_SUBMISSION lifecycle status to the submission, claim and bulk_submission
-- status CHECK constraints. A submission (and its claims / bulk submission) moves to
-- READY_FOR_SUBMISSION when it passes INITIAL validation and is held for provider review before
...
ALTER TABLE claim
    ADD CONSTRAINT chk_claim_status CHECK (status IN ('READY_TO_PROCESS', 'VALID', 'INVALID', 'VOID', 'READY_FOR_SUBMISSION'));
