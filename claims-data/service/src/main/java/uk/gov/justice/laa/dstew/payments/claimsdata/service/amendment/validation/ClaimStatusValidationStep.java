 ...
 * rejected with {@link ClaimAmendmentValidationCode#INVALID_VOIDED_CLAIM_NOT_AMENDABLE}; any other
 * non-{@code VALID} status is rejected with {@link
 * ClaimAmendmentValidationCode#INVALID_CLAIM_STATE_NOT_AMENDABLE}. Both rejections are fatal, so no
...
public class ClaimStatusValidationStep implements ClaimAmendmentValidationStep {
  ...
  public List<ClaimAmendmentValidationError> validate(ClaimAmendmentState state) {
    ...
    return List.of(
        ClaimAmendmentValidationError.of(
            ClaimAmendmentValidationCode.INVALID_CLAIM_STATE_NOT_AMENDABLE,
            status,
            ClaimStatus.VALID));
  }
}
