...
/**
 * This class provide basic filtering logic to query {@link Submission} entities using JPA {@link Specification}.
 *
 * <p>The resulting specification can be used with Spring Data JPA repositories to fetch
 * filtered submission records from the database.</p>
 */
public final class SubmissionSpecification {
   ...
   * various parameters. The resulting specification can be used to dynamically generate
   * predicates for querying submissions.
   *
   ...
   * @return a JPA {@code Specification} of {@code Submission} containing the constructed filtering predicates
   */
  public static Specification<Submission> filterBy(
    ...
    return (Root<Submission> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
      ...
      if (submittedDateFrom != null) {
        predicate = cb.and(predicate, cb.greaterThanOrEqualTo(root.get("createdOn"), submittedDateFrom));
      }

      if (submittedDateTo != null) {
        // Criteria builder is converting dates into a format with time component at the start of the day (e.g. '2025-09-01 00:00:00'),
        // which means a submission "created on" 2025-09-01 19:05:38 is not going to satisfy the "lessThanOrEqualTo" predicate,
