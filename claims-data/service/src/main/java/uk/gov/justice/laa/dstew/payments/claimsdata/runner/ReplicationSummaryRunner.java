...
public class ReplicationSummaryRunner implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    for (Map<String, Object> table : tables) {
      ...
      log.info("Processing table: {}", fullName);

      // Build SQL dynamically
      String countSql = String.format("SELECT count(*) FROM %s WHERE created_on < ?", fullName);
      String updatedSql =
          String.format("SELECT count(*) FROM %s WHERE updated_on BETWEEN ? AND ?", fullName);

      Long recordCount = jdbcTemplate.queryForObject(countSql, Long.class, endOfDay);
      Long updatedCount = jdbcTemplate.queryForObject(updatedSql, Long.class, startOfDay, endOfDay);
      ...
      jdbcTemplate.update(upsertSql, fullName, summaryDate, recordCount, updatedCount);
    }
