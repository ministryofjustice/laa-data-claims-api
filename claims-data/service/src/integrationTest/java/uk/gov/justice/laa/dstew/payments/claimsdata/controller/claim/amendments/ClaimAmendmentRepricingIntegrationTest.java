...
class ClaimAmendmentRepricingIntegrationTest extends MockServerIntegrationTest {
  ...
  void shouldSuccessfullyRepriceAndCommitValidAmendment() throws Exception {
    ...
    String mockResponseBody = "{\"feeCode\":\"FEE-123\",\"schemeId\":\"SCHEME-TEST\",\"escapeCaseFlag\":false,\"feeCalculation\":{\"totalAmount\":650.00,\"netProfitCostsAmount\":450.00,\"vatIndicator\":true}}";

    mockServerClient
        .when(request().withMethod("POST").withPath(FEE_CALCULATION_PATH))
        .respond(response().withStatusCode(200).withContentType(MediaType.APPLICATION_JSON).withBody(mockResponseBody));
    ...
    List<CalculatedFeeDetail> savedFees = calculatedFeeDetailRepository.findAll().stream()
  ...
  void shouldReturnBadRequestWhenFspValidationFails() throws Exception {
    ...
    mockServerClient
        .when(request().withMethod("POST").withPath(FEE_CALCULATION_PATH))
        .respond(response().withStatusCode(400).withBody("Invalid profit cost configuration combo"));
  ...
  void shouldReturnServiceUnavailableOnFspNetworkTimeout() throws Exception {
    ...
    assertThat(mvcResult.getResponse().getContentAsString())
        .contains("A technical error occurred while recalculating the fee");
  }

  @Test
  @DisplayName("PATCH /submissions/{id}/claims/{id} - skips FSP repricing when changes do not impact pricing")
  ...
  @Test
  @DisplayName("PATCH /submissions/{id}/claims/{id} - skips FSP repricing when baseline lacks calculated fee details")
  ...
  @Test
  @DisplayName("PATCH /submissions/{id}/claims/{id} - returns 503 Service Unavailable when FSP returns 500")
  ...
  @Test
  @DisplayName("PATCH /submissions/{id}/claims/{id} - returns 503 Service Unavailable when FSP body is null")
  ...
  private void createCalculatedFeeDetail(
    ...
    ClaimSummaryFee summaryFee = ClaimSummaryFee.builder()
        .claim(claim)
